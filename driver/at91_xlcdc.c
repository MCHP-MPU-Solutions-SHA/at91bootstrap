...
#define XLCDC_BMP_ADDR (CONFIG_LOGO_FB_ADDRESS + 16)
...
static void xlcdc_show_base(void)
{
	...
	xlcdc_writel(LCDC_BASECFG(3), LAYER_RGB(CONFIG_LOGO_RGBDEF));
...
static void xlcdc_show_heo(void)
{
	...
	xlcdc_writel(LCDC_HEOCFG(9), LAYER_RGB(CONFIG_LOGO_RGBDEF));
...
#endif
	xlcdc_set_backlight(CONFIG_LOGO_BL);
}
...
UNSUPPORTED:
	...
	xlcdc.ovr_sc_width  = (CONFIG_LOGO_SCALE == 0) ? xlcdc.ovr_width : (xlcdc.ovr_width * (8 + CONFIG_LOGO_SCALE) / 8);
	xlcdc.ovr_sc_height = (CONFIG_LOGO_SCALE == 0) ? xlcdc.ovr_height : (xlcdc.ovr_height * (8 + CONFIG_LOGO_SCALE) / 8);
