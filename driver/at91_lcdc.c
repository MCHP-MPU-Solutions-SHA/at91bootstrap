...
#define LCDC_DMA_ADDR CONFIG_LOGO_FB_ADDRESS
#define LCDC_BMP_ADDR (CONFIG_LOGO_FB_ADDRESS + sizeof(struct dma_desc))
...
void lcdc_show_base(void)
{
	...
	lcdc_writel(LCDC_BASECFG(3), LAYER_RGB(CONFIG_LOGO_RGBDEF));
...
void lcdc_show_heo(void)
{
	...
	lcdc_writel(LCDC_HEOCFG(9), LAYER_RGB(CONFIG_LOGO_RGBDEF));
...
void lcd_init(void)
{
	...
	if (CONFIG_LOGO_BL_DELAY)
		mdelay(CONFIG_LOGO_BL_DELAY);
	lcdc_set_backlight(CONFIG_LOGO_BL);
}
...
UNSUPPORTED:
	...
	lcdc.ovr_sc_width  = (CONFIG_LOGO_SCALE == 0) ? lcdc.ovr_width : (lcdc.ovr_width * (8 + CONFIG_LOGO_SCALE) / 8);
	lcdc.ovr_sc_height = (CONFIG_LOGO_SCALE == 0) ? lcdc.ovr_height : (lcdc.ovr_height * (8 + CONFIG_LOGO_SCALE) / 8);
