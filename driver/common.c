...
void init_load_image(struct image_info *image)
{
	...
	image->length = CONFIG_UNC_IMAGE_SIZE;
#endif
	...
	image->logo_offset = CONFIG_LOGO_ADDRESS;
#endif
#ifdef CONFIG_SDCARD
	image->logo_filename = logo_filename;
	strcpy(image->logo_filename, CONFIG_LOGO_NAME);
#endif
	image->logo_dest = (unsigned char *)(CONFIG_LOGO_FB_ADDRESS + 16);
#endif
}
